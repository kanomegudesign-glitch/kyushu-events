...
# バッジ用の既知の市町村名（接尾辞なしでも拾えるように）
_KNOWN_CITIES = [
    "久留米", "大牟田", "筑後", "八女", "柳川", "大川", "みやま", "小郡",
    "広川", "大木", "うきは", "大刀洗", "筑紫野", "朝倉",
    "玉名", "山鹿", "荒尾", "南関", "長洲", "和水", "玉東", "菊水",
    "佐賀", "鳥栖", "みやき", "神埼", "基山", "上峰",
]


def _city_label(e):
    """バッジ用の短い地名（久留米市→久留米／イオンモール大牟田→大牟田）。"""
    place, title = e.get("place") or "", e.get("title") or ""
    source = e.get("source") or ""
    for src in (place, title):
        m = re.search(r"([一-龥ぁ-んァ-ヶ]{2,5})(市|町|村)", src)
        if m:
            return m.group(1)
    for src in (place, title, source):
...
# 既存の2件構成テンプレートのフィールド接続に合わせて列見出しを固定。
CANVA_HEADERS = [
    "見出し日付",
...
# CSV（投稿候補）に載せない長期開催の上限日数。これを超える販促・展示・
# 通年企画は除外し、単発の“映える”イベントを並べる。
...
def build_canva_csv(events, path="events_canva.csv", max_rows=15):
    """開催日順に2件ずつペアにして、Canva一括作成用CSVを書き出す（長期開催は除外）。"""
    items = []
    for e in events:
        if not e.get("start"):
            continue
...
def main():
    ...
    n_csv = build_canva_csv(filtered)
    print(f"[DONE] events_canva.csv を書き出しました（{n_csv}行）", file=sys.stderr)
